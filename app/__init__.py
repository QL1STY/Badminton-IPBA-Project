...
# --- Logika do ustawiania pierwszego admina ---
with app.app_context():
    admin_email = os.environ.get('ADMIN_EMAIL')
    if admin_email:
        # Importujemy model User dopiero tutaj, gdy jest potrzebny
        user = models.User.query.filter_by(email=admin_email).first()
        if user and not user.is_admin:
            user.is_admin = True
            db.session.commit()
            print(f"Nadano uprawnienia administratora użytkownikowi z adresem e-mail: {admin_email}")