...
class RegistrationForm(FlaskForm):
    ...
    email = StringField(
        ...
        validators=[
            DataRequired(),
            Email(message=_l("Niepoprawny format adresu email.")),
        ],
    )
    password = PasswordField(
        _l("Hasło"), validators=[DataRequired(), validate_password_strength]
    )
    confirm_password = PasswordField(
        _l("Potwierdź hasło"),
        validators=[
            DataRequired(),
            EqualTo("password", message=_l("Hasła muszą być identyczne.")),
        ],
    )
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError(
                _l("Ta nazwa użytkownika jest już zajęta. Proszę wybrać inną.")
            )
    ...
    def validate_email(self, email):
        ...
        if user:
            raise ValidationError(
                _l("Ten adres email jest już zajęty. Proszę wybrać inny.")
            )


class LoginForm(FlaskForm):
    login_identifier = StringField(
        _l("Nazwa użytkownika lub Email"), validators=[DataRequired()]
    )
    password = PasswordField(_l("Hasło"), validators=[DataRequired()])
...
class PostForm(FlaskForm):
    title = StringField(
        _l("Tytuł"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
    content = TextAreaField(
        _l("Treść"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
    picture = FileField(
        _l("Zdjęcie nagłówkowe (baner)"),
...
class ContactForm(FlaskForm):
    name = StringField(
        _l("Twoje Imię"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
    email = StringField(
        _l("Twój Email"),
        filters=[bleach_clean_text],
    ...
    subject = StringField(
        _l("Temat"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
    message = TextAreaField(
        _l("Wiadomość"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
...
class ResetPasswordForm(FlaskForm):
    password = PasswordField(
        _l("Nowe hasło"), validators=[DataRequired(), validate_password_strength]
    )
    confirm_password = PasswordField(
        _l("Potwierdź nowe hasło"),
        validators=[
            DataRequired(),
            EqualTo("password", message=_l("Hasła muszą być identyczne.")),
        ],
...
class UpdateAccountForm(FlaskForm):
    first_name = StringField(
        _l("Imię"), validators=[DataRequired(), Length(min=2, max=30)]
    )
    last_name = StringField(
        _l("Nazwisko"), validators=[DataRequired(), Length(min=2, max=30)]
    )
    username = StringField(
        _l("Nazwa użytkownika"),
        validators=[DataRequired(), Length(min=2, max=20), validate_username_profanity],
...
class ChangePasswordForm(FlaskForm):
    ...
    confirm_password = PasswordField(
        ...
        validators=[
            DataRequired(),
            EqualTo("new_password", message=_l("Hasła muszą być identyczne.")),
        ],
    )
...
class TournamentForm(FlaskForm):
    ...
    description = TextAreaField(
        _l("Opis turnieju"), filters=[bleach_clean_text], validators=[DataRequired()]
    )
    banner_image = FileField(
        _l("Baner turnieju"), validators=[FileAllowed(["jpg", "png", "jpeg"])]
    )
    ...
    start_date = DateField(
        _l("Data rozpoczęcia"), format="%Y-%m-%d", validators=[DataRequired()]
    )
    end_date = DateField(
        _l("Data zakończenia (opcjonalnie)"),
        format="%Y-%m-%d",
        validators=[DataRequired()],
    )
    max_players = IntegerField(
        _l("Maksymalna liczba graczy"), validators=[DataRequired()]
    )
