...
from app.forms import (RegistrationForm, LoginForm, PostForm, ContactForm, 
...
@app.route('/index')
def index():
    # ZMIANA: Zamiast .paginate(), używamy .limit(3) do pobrania tylko 3 postów
    posts = Post.query.order_by(Post.date_posted.desc()).limit(3).all()
    
    today = datetime.utcnow().date()
    # ZMIANA: Używamy .limit(3) do pobrania tylko 3 nadchodzących i 3 przeszłych turniejów
    upcoming_tournaments = Tournament.query.filter(Tournament.start_date >= today).order_by(Tournament.start_date.asc()).limit(3).all()
    past_tournaments = Tournament.query.filter(Tournament.start_date < today).order_by(Tournament.start_date.desc()).limit(3).all()

    return render_template('index.html', title=_('Strona Główna'),
                           posts=posts,  # ZMIANA: Przekazujemy listę, a nie obiekt paginacji
...
def rejestracja():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data,
                    password_hash=hashed_password, first_name=form.first_name.data,
                    last_name=form.last_name.data, email_verified=False) 
        db.session.add(user)
        db.session.commit()
        ...
        send_email('Potwierdź swój adres email', [user.email], 'Potwierdź swój adres email', html)

        flash(_('Konto zostało utworzone! Link weryfikacyjny został wysłany na Twój email.'), 'success')
        return redirect(url_for('logowanie'))
    return render_template('register.html', title=_('Rejestracja'), form=form)

# --- NOWA TRASA DO WERYFIKACJI EMAILA ---
...
def verify_email(token):
    ...
    if user:
        if user.email_verified:
            ...
            db.session.commit()
            flash(_('Twój email został pomyślnie zweryfikowany! Możesz się teraz zalogować.'), 'success')
    else:
...
def logowanie():
    ...
    form = LoginForm()
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.login_identifier.data).first() or User.query.filter_by(username=form.login_identifier.data).first()

        if user and check_password_hash(user.password_hash, form.password.data):
            # SPRAWDZAMY, CZY EMAIL JEST ZWERYFIKOWANY
            if not user.email_verified:
                flash(_('Twoje konto nie zostało jeszcze zweryfikowane. Sprawdź swoją skrzynkę mailową.'), 'warning')
                return redirect(url_for('logowanie'))
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash(_('Jeśli konto istnieje, instrukcje resetowania hasła zostały wysłane na email.'), 'info')
        return redirect(url_for('logowanie'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Twoje hasło zostało zaktualizowane! Możesz się teraz zalogować.'), 'success')
        return redirect(url_for('logowanie'))
...
def kontakt():
    form = ContactForm()
    # Sprawdzamy, czy użytkownik jest zalogowany i czy metoda to GET
    # (aby nie nadpisywać danych z formularza przy nieudanym POST)
    ...
    if form.validate_on_submit():
        try:
            ...
            flash(_('Wystąpił błąd podczas wysyłania wiadomości. Spróbuj ponownie później.'), 'danger')
...
# --- WIDOKI DO ZARZĄDZANIA POSTAMI (DLA ADMINA) ---
    ...
    if form.validate_on_submit():
        ...
        if form.picture.data:
            ...
            if saved_filename:
                ...
                flash(_('Przesłany plik nie jest prawidłowym obrazem (dozwolone formaty: JPG, PNG).'), 'danger')
                return render_template('create_post.html', title=_('Nowy Post'), form=form)

        # --- NOWA LINIA: Czyszczenie HTML ---
        cleaned_content = bleach.clean(form.content.data, tags=['p', 'br', 'b', 'i', 'u', 'strong', 'em', 'a'], attributes={'a': ['href', 'title']})

        post = Post(title=form.title.data, content=cleaned_content, author=current_user, image_file=image_filename)
        db.session.add(post)
        db.session.commit()
...
def post(post_id):
    ...
    return render_template('post.html', title=post.title, post=post, delete_form=delete_form)
...
def profil():
    ...
    days_left = None
    # Obliczamy pozostały czas tylko wtedy, gdy data ostatniej zmiany istnieje
    if current_user.username_last_changed and not current_user.can_change_username():
        ...
        # Używamy ceil, aby zaokrąglić w górę, np. 13.5 dnia do 14 dni
        days_left = ceil(time_remaining.total_seconds() / (24 * 3600))


    # Logika dla formularza zmiany DANYCH OSOBOWYCH
    if update_form.validate_on_submit() and update_form.submit_details.data:
        ...
        if current_user.username != update_form.username.data:
            if current_user.can_change_username():
                ...
                flash(_('Nazwę użytkownika można zmieniać tylko raz na 14 dni.'), 'warning')
    ...
    # Logika dla formularza zmiany HASŁA
    ...
    # Przy wejściu na stronę (metoda GET) wypełniamy formularz aktualnymi danymi
    if request.method == 'GET':
        update_form.first_name.data = current_user.first_name
        update_form.last_name.data = current_user.last_name
        update_form.username.data = current_user.username
    
    return render_template('profile.html', title=_('Profil Użytkownika'), 
                           update_form=update_form, password_form=password_form, days_left=days_left)
...
def tournaments():
    """Wyświetla listę wszystkich turniejów."""
    all_tournaments = Tournament.query.order_by(Tournament.start_date.desc()).all()
    return render_template('tournaments.html', title=_('Turnieje'), tournaments=all_tournaments)

@app.route('/tournament/<int:tournament_id>')
def tournament_details(tournament_id):
    """Wyświetla szczegóły pojedynczego turnieju."""
    tournament = Tournament.query.get_or_404(tournament_id)
    registrations = tournament.registrations.all()
    # Przekazujemy `asc` i `datetime` do szablonu
    return render_template('tournament_details.html', title=tournament.title, 
...
def register_for_tournament(tournament_id):
    """Zapisuje użytkownika na turniej."""
    tournament = Tournament.query.get_or_404(tournament_id)
    
    # --- NOWA LOGIKA: Sprawdzenie daty turnieju ---
...
def unregister_from_tournament(tournament_id):
    """Wypisuje użytkownika z turnieju."""
    tournament = Tournament.query.get_or_404(tournament_id)
    ...
    registration = TournamentRegistration.query.filter_by(user_id=current_user.id, tournament_id=tournament_id).first_or_404()
...
# ======================================================
# --- NOWA SEKCJA: OBSŁUGA BŁĘDÓW ---
# ======================================================

@app.errorhandler(404)
def error_404(error):
    """Renderuje stronę dla błędu 404 - Nie znaleziono strony."""
    return render_template('errors/404.html', title=_('Nie znaleziono strony')), 404

@app.errorhandler(403)
def error_403(error):
    """Renderuje stronę dla błędu 403 - Brak dostępu."""
...
@admin_required
def admin_dashboard():
    """Główny widok panelu administratora."""
    user_count = User.query.count()
    post_count = Post.query.count()
    # Dodajemy zliczanie turniejów
    tournament_count = Tournament.query.count()
    return render_template('admin/dashboard.html', title=_('Panel Administratora'),
                           user_count=user_count, 
                           post_count=post_count,
                           tournament_count=tournament_count) # Przekazujemy nową zmienną
@app.route('/admin/users')
@login_required
@admin_required
def admin_manage_users():
    """Strona do zarządzania użytkownikami."""
    # Pobieramy wszystkich użytkowników, na początku listy pokazujemy adminów
    users = User.query.order_by(User.is_admin.desc(), User.id).all()
    delete_form = DeleteForm()
    return render_template('admin/manage_users.html', title=_('Zarządzaj Użytkownikami'), users=users, delete_form=delete_form)

@app.route('/admin/user/<int:user_id>/toggle_admin_confirm', methods=['GET', 'POST'])
@login_required
@admin_required
def admin_toggle_admin_confirm(user_id):
    """Wyświetla stronę z prośbą o potwierdzenie hasłem zmiany uprawnień."""
    ...
    if form.validate_on_submit():
        if check_password_hash(current_user.password_hash, form.password.data):
            ...
            flash(_('Pomyślnie %(status)s uprawnienia administratora użytkownikowi %(username)s.', 
                    status=status, username=user_to_modify.username), 'success')
            return redirect(url_for('admin_manage_users'))
        else:
            flash(_('Nieprawidłowe hasło. Operacja anulowana.'), 'danger')
            return redirect(url_for('admin_manage_users'))

    return render_template('admin/confirm_admin_toggle.html', 
...
def admin_delete_user(user_id):
    """Usuwa użytkownika i wszystkie jego posty."""
    form = DeleteForm()
    user_to_delete = User.query.get_or_404(user_id)
    if form.validate_on_submit():
        if user_to_delete.id == current_user.id:
            flash(_('Nie możesz usunąć własnego konta z panelu administratora.'), 'danger')
            return redirect(url_for('admin_manage_users'))
        Post.query.filter_by(author=user_to_delete).delete()
        db.session.delete(user_to_delete)
        db.session.commit()
        flash(_('Użytkownik %(username)s i wszystkie jego posty zostały usunięte.', username=user_to_delete.username), 'success')
    else:
...
def admin_manage_posts():
    """Strona do zarządzania wszystkimi postami."""
    # Pobieramy wszystkie posty, od najnowszych
    posts = Post.query.order_by(Post.date_posted.desc()).all()
    delete_form = DeleteForm()
    return render_template('admin/manage_posts.html', title=_('Zarządzaj Postami'), posts=posts, delete_form=delete_form)
...
@app.route('/admin/tournament/<int:tournament_id>/delete_registration/<int:user_id>', methods=['POST'])
@login_required
@admin_required
def delete_registration(tournament_id, user_id):
    """Usuwa zapis użytkownika z turnieju."""
    registration = TournamentRegistration.query.filter_by(user_id=user_id, tournament_id=tournament_id).first_or_404()
    db.session.delete(registration)
    db.session.commit()
...
def admin_manage_tournaments():
    """Wyświetla listę wszystkich turniejów w panelu admina."""
    tournaments = Tournament.query.order_by(Tournament.start_date.desc()).all()
    delete_form = DeleteForm()
    return render_template('admin/manage_tournaments.html', title=_('Zarządzaj Turniejami'), tournaments=tournaments, delete_form=delete_form)

@app.route('/admin/tournament/<int:tournament_id>/update', methods=['GET', 'POST'])
@login_required
@admin_required
def admin_update_tournament(tournament_id):
    """Edytuje istniejący turniej."""
    tournament = Tournament.query.get_or_404(tournament_id)
    form = TournamentForm()
    if form.validate_on_submit():
        if form.banner_image.data:
            ...
            if saved_filename:
                ...
                flash(_('Przesłany plik nie jest prawidłowym obrazem (dozwolone formaty: JPG, PNG).'), 'danger')
                return render_template('create_tournament.html', title=_('Edytuj Turniej'), form=form)
...
def admin_delete_tournament(tournament_id):
    """Usuwa turniej i wszystkie powiązane z nim zapisy."""
    form = DeleteForm()
    if form.validate_on_submit():
        tournament = Tournament.query.get_or_404(tournament_id)
...
@app.route('/admin/tournament/<int:tournament_id>/manage_winners', methods=['GET', 'POST'])
@login_required
@admin_required
def admin_manage_winners(tournament_id):
    """Strona do zarządzania zwycięzcami danego turnieju."""
    tournament = Tournament.query.get_or_404(tournament_id)
    form = AddWinnerForm()
    ...
    return render_template('admin/manage_winners.html', title=_('Zarządzaj Zwycięzcami'),
                           form=form, tournament=tournament, winners=winners)

@app.route('/admin/winner/<int:winner_id>/delete', methods=['POST'])
@login_required
@admin_required
def admin_delete_winner(winner_id):
    """Usuwa wpis o zwycięzcy."""
